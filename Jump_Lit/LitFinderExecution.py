...
x=os.getcwd()
params= x + '/' + "jump_lit.params"
print(params)
log_file="log_file.txt"
def Write_Log_File(log_file='log_file.txt', log_string=''):
    f=open("log_file.txt", "a")
    x=f.write(log_string+ "\n\n")
...
def Accessing_User_Input(Input_Type, user_input_file = None, user_input_genes = None):
    print("user input file is", user_input_file)
    if Input_Type == 0:                                                             #Verifies if the input type is a list based on the input of the parameter file
        ...
        content=items.manipulating_list()
        item_list=content[0]                                      #If the input is a list, the manipulatin_list function from Organizing_User_Input is called and indexed at position 0 to acquire the gene list
        item_number=content[1]                                    #If the input is a list, the manipulating_list function from Organizing_User_Input is called and indexed at position 1 to derive the number of genes to be investigated                                   
        Write_Log_File(log_file, "The list of genes being searched is %s" %item_list)
        print("The list of genes being searched is %s" %item_list)
        ...
        content=items.manipulating_individuals()
        item_list=content[0]                               #If the input is a manual input, the manipulating_individual function from Organizing_User_Input is called and indexed at position 0 to acquire the gene list
        item_number=content[1]                              #If the input is a manual input, the manipulating_individual function from Organizing_User_Input is called and indexed at position 0 to derive the number of genes to be investigated
...
def Making_URL_1(Gene_List, FilterOption):
    ...
    Write_Log_File(log_file, "The list of ESearch_URLs is displayed here %s" %ESearch_URL_List)
    print("The ESearch URLs for each gene have been generated")
...
def AccessingEsearchDatabase(Esearch_URL_List):
    ...
    Write_Log_File(log_file, "Access to each ESearch URL has been obtained and the content thereof has been stored %s" %ESearch_Content)
    print("Access to each ESearch URL has been obtained and the content thereof is stored in the log file")
...
def Making_EFetch_URL(ESearch_Content, outputlimit):
    ...
    Write_Log_File(log_file, "The Web Environment has been parsed from the ESearch URL and has been used to produce the EFetch URLs which have been stored here %s" %EFetch_URL_List)
    print("The Web Environment has been parsed from the ESearch URL and has been used to produce the EFetch URLs which have been stored in the log file")
...
def Generating_Literature_Data(EFetch_URL_List, outputlimit):
    ...
    Write_Log_File(log_file, "The DOIs and PMIDs for each gene have been acquired and used to develop the dataframe. The DOI and PMID lists along with the dataframe, have been stored here %s %s %s" %(Flat_DOI_List, Flat_PMID_List, dataarray))
    print("The DOIs and PMIDs for each gene have been acquired and used to develop the dataframe. The DOI and PMID lists, along with the dataframe, have been stored in the log file")
...
def Generating_Bar_Graph(Gene_List, dataarray, DOI_Num):                            #The GeneratingBarGraph function operates similarly but rather, produces bar graph rather than general frame
    ...
    Write_Log_File(log_file, "The dataframe associating each gene with its respective DOI and PMID, as well as the bar graph of data, has been produced and stored here: %s %s" %(df, bar_graph))
    print("The dataframe associating each gene with its respective DOI and PMID, as well as the bar graph of data, has been produced and stored in the log file")
...
def Accessing_PMID_File(Flat_PMID_List):                                                                          
    ...
    Write_Log_File(log_file, "The PMIDs have been separated into batches, used to construct EPost URLs, and the content of EPost URLs has been stored here along with the EPost Url List %s %s" %(EPost_URLs, Content_Citation_Data))
    print("The PMIDs have been separated into batches, used to construct EPost URLs, and the content of EPost URLs has been stored in the log file, along with the EPost Url List")
...
def Generating_Esummary_URLs(Content_EPost_Data, output_limit):
    ...
    Write_Log_File(log_file, "The Web environment has been parsed from the EPost URL and used to generate the ESummary URLs, which are stored here: %s" %ESummary_URL_List)
    print("The Web environment has been parsed from the EPost URL and used to generate the ESummary URLs, which are stored in the log file")
...
def Generating_Citation_Data(PMID_Citation_List, PMID_List, output_limit):
    ...
    Write_Log_File(log_file, "The citation data has been successfully parsed from the ESummary URL and used to generate the citation dataframe and bar graph of citation data, which are both stored here: %s %s" %(PMID_df, PMID_bar_graph))
    print("The citation data has been successfully parsed from the ESummary URL and used to generate the citation dataframe and bar graph of citation data, which are both stored in the log file")
...
def MetaAnalysis(Citation_Number_List, Flat_PMID_List, Large_PMID_List, citation_limit):
    ...
    Write_Log_File(log_file, "The citation data frame returning the most cited papers for each gene has been created and is stored here: %s" %New_Citation_DataFrames)
    print("The citation data frame returning the most cited papers for each gene has been created and is stored in the log file")
...
def Citation_Data_Frame(FileName2, df, bar_graph, PMID_df, PMID_bar_graph, New_Citation_DataFrame, citation_ctr, num_genes, output_location):
    ...
    import random
    y=str(random.random())
    x=os.mkdir(output_location+y)
    os.rename(FileName2, output_location+y+"/"+FileName2)
    os.rename(log_file, output_location+y+'/'+log_file)
    
    
    

    Write_Log_File(log_file, "All data has been successfully exported and the program has finished running")
    print("All data has been successfully exported and the program has finished running")
##########################################################################################################
############################ Execution of the Program ####################################################
##########################################################################################################
import parser_v2 
param_file=parser_v2.parserParams(params)
print('input type is', param_file['input_type'])
maxima=9999999999999999                                                                                 #Maxima serves as an automatic input for the output limit
User_Input_Info=Accessing_User_Input(int(param_file['input_type']), param_file['user_input_file']) #This calls the Accessing_User_Input function above, specifying the input type from the parameter file 
...
ESearch_URL_List = Making_URL_1(L, int(param_file['filter_option']))                                    #The Making_URL_1 function is called using the gene list and includes the option for filtering as stipulated by the LitFinderParameterFile
...
if int(param_file['output_limit'])==0:                                                                  #In the event that the user does not impose an output limit via the LitFinderParameter file...
...
elif int(param_file['output_limit'])==1:                                                                #In the event that the user imposes an output limit via the LitFinderParameterFile... 
    All_Content=Generating_Literature_Data(EFetch_URL_List, int(param_file['output_limits']))           #The Generating_URL_Function is called on the list of EFetch URLs and imposes a limit on output
...
if int(param_file['citation_data']) == 0:                                                               #If the user elects to receive citation data on the papers procured
    ...
    ESummary_List=Generating_Esummary_URLs(Content_EPost_Data, int(param_file['output_limits']))        #The Generating_ESummary_URLs function is called on the output of the EPost URLs and imposes a limit specified via the LitFinderParameterFile
    ...
    Citation_Data_Frame(param_file['output_file'], df, bar_graph, PMID_df, PMID_bar_graph,         #The Citation_Data_Frame function is called on the comprehensive list of data and exports the data
                        New_Content_Citation_Data, citation_ctr, num_genes, param_file['output_folder'])                             #The Citation_Data_Frame function is called on the comprehensive list of data and exports the data
elif int(param_file['citation_data'])==1:                                                               #If, however, the user elects to skip citation data...
    Citation_Data_Frame(param_file['output_file'], df, bar_graph, citation_ctr, num_genes)         #The program ends prematurely, exporting only the raw data 
##########################################################################################################
